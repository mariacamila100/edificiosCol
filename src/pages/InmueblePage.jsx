    ...
    <div className="max-w-[1600px] mx-auto px-6 py-10 min-h-screen bg-[#fafafa]">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
        <div className="space-y-1">
          <h1 className="text-2xl font-medium text-slate-900 tracking-tight">Inventario de Inmuebles</h1>
          <p className="text-sm text-slate-500">Panel de administración y control de activos.</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden transition-all">
        <div className="p-4 border-b border-slate-100 flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1 group">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-transparent focus:bg-white focus:border-slate-300 rounded-lg text-sm transition-all outline-none text-slate-600"
              value={searchTerm} 
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <select
                ...
                className="pl-10 pr-8 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 outline-none hover:border-slate-300 appearance-none cursor-pointer"
              >
        ...
        {/* TABLA SIN NEGRILLAS PESADAS */}
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-left">Título y Ubicación</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-center">Hab.</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-center">Baños</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-center">Área (m²)</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-center">Estado</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-right">Precio Unitario</th>
                <th className="px-6 py-4 text-[11px] font-medium text-slate-400 uppercase tracking-wider text-right">Acciones</th>
              </tr>
            </thead>
                ...
                <tr key={item.id} className="hover:bg-blue-50/20 transition-all group">
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="text-sm text-slate-700 line-clamp-1">{item.titulo}</span>
                      <span className="text-[11px] text-slate-400 mt-0.5">{item.barrio} {item.nombreEdificio ? `· ${item.nombreEdificio}` : ''}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 text-center">
                    <span className="text-xs text-slate-500">{item.habitaciones}</span>
                  </td>
                  <td className="px-6 py-5 text-center">
                    <span className="text-xs text-slate-500">{item.baños}</span>
                  </td>
                  <td className="px-6 py-5 text-center">
                    <span className="text-xs font-mono text-slate-400">{item.area} m²</span>
                  </td>
                  <td className="px-6 py-5 text-center">
                    <span className={`px-2.5 py-1 rounded-md text-[10px] tracking-wider ${
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <button 
              ...
              className="p-2 border border-slate-200 rounded-lg disabled:opacity-20 hover:bg-white transition-all text-slate-500 shadow-sm"
            >
              <ChevronLeft size={16} />
            </button>
            <div className="flex gap-1 px-2">
                {[...Array(totalPages)].map((_, i) => (
                    <button 
                        key={i}
                        onClick={() => setPage(i + 1)}
                        className={`w-8 h-8 text-xs rounded-lg transition-all ${page === i + 1 ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-200'}`}
                    >
                        {i + 1}
                    </button>
                ))}
